...
func (f *Func) bodyToCSharp() ([]string, error) {
	...
	for _, instr := range dis.Code {
		...
		case operators.I32Load:
			...
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("int stack%d = mem_.LoadInt32(stack%d + %d);", idx, addr, offset)
		case operators.I64Load:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("long stack%d = mem_.LoadInt64(stack%d + %d);", idx, addr, offset)
		case operators.F32Load:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("float stack%d = mem_.LoadFloat32(stack%d + %d);", idx, addr, offset)
		case operators.F64Load:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("double stack%d = mem_.LoadFloat64(stack%d + %d);", idx, addr, offset)
		case operators.I32Load8s:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("int stack%d = (int)mem_.LoadInt8(stack%d + %d);", idx, addr, offset)
		case operators.I32Load8u:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("int stack%d = (int)mem_.LoadUint8(stack%d + %d);", idx, addr, offset)
		case operators.I32Load16s:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("int stack%d = (int)mem_.LoadInt16(stack%d + %d);", idx, addr, offset)
		case operators.I32Load16u:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("int stack%d = (int)mem_.LoadUint16(stack%d + %d);", idx, addr, offset)
		case operators.I64Load8s:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("long stack%d = (long)mem_.LoadInt8(stack%d + %d);", idx, addr, offset)
		case operators.I64Load8u:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("long stack%d = (long)mem_.LoadUint8(stack%d + %d);", idx, addr, offset)
		case operators.I64Load16s:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("long stack%d = (long)mem_.LoadInt16(stack%d + %d);", idx, addr, offset)
		case operators.I64Load16u:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("long stack%d = (long)mem_.LoadUint16(stack%d + %d);", idx, addr, offset)
		case operators.I64Load32s:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("long stack%d = (long)mem_.LoadInt32(stack%d + %d);", idx, addr, offset)
		case operators.I64Load32u:
			offset := instr.Immediates[1].(uint32)
			addr := idxStack.Pop()
			idx := idxStack.Push()
			appendBody("long stack%d = (long)mem_.LoadUint32(stack%d + %d);", idx, addr, offset)

		case operators.I32Store:
			offset := instr.Immediates[1].(uint32)
			idx := idxStack.Pop()
			addr := idxStack.Pop()
			appendBody("mem_.StoreInt32(stack%d + %d, stack%d);", addr, offset, idx)
		case operators.I64Store:
			offset := instr.Immediates[1].(uint32)
			idx := idxStack.Pop()
			addr := idxStack.Pop()
			appendBody("mem_.StoreInt64(stack%d + %d, stack%d);", addr, offset, idx)
		case operators.F32Store:
			offset := instr.Immediates[1].(uint32)
			idx := idxStack.Pop()
			addr := idxStack.Pop()
			appendBody("mem_.StoreFloat32(stack%d + %d, stack%d);", addr, offset, idx)
		case operators.F64Store:
			offset := instr.Immediates[1].(uint32)
			idx := idxStack.Pop()
			addr := idxStack.Pop()
			appendBody("mem_.StoreFloat64(stack%d + %d, stack%d);", addr, offset, idx)
		case operators.I32Store8:
			offset := instr.Immediates[1].(uint32)
			idx := idxStack.Pop()
			addr := idxStack.Pop()
			appendBody("mem_.StoreInt8(stack%d + %d, (sbyte)stack%d);", addr, offset, idx)
		case operators.I32Store16:
			offset := instr.Immediates[1].(uint32)
			idx := idxStack.Pop()
			addr := idxStack.Pop()
			appendBody("mem_.StoreInt16(stack%d + %d, (short)stack%d);", addr, offset, idx)
		...
		case operators.I64Store32:
			...
			idx := idxStack.Pop()
			addr := idxStack.Pop()
			appendBody("mem_.StoreInt32(stack%d + %d, (int)stack%d);", addr, offset, idx)
		...
		case operators.GrowMemory:
			delta := idxStack.Pop()
			dst := idxStack.Push()
			appendBody("int stack%d = mem_.Grow(stack%d);", dst, delta)

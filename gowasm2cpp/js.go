...
std::unordered_set<int32_t>& GetEmptyArrayIdPool() {
  static std::unordered_set<int32_t> ids;
  return ids;
}

int32_t GetNextEmptyArrayId() {
  static int32_t next_id_ = 0;

  auto& pool = GetEmptyArrayIdPool();
  if (pool.size()) {
    int32_t id = *pool.begin();
    pool.erase(id);
    return id;
  }
  next_id_++;
...
void PutEmptyArrayId(int32_t id) {
  auto& pool = GetEmptyArrayIdPool();
  pool.insert(id);
}
...
Value::Value(const std::vector<Value>& array)
  ...
  if (!array.size()) {
    // Two nil array values should be 'different'. Give an id to make this value unique.
    num_value_ = GetNextEmptyArrayId();
  }
}
...
std::vector<Value>& Value::ToMutableArray() {
  ...
  if (!array_value_) {
    ...
    PutEmptyArrayId(num_value_);
    // num_value_ is an indentifier to make this value unique for a nil array.
    // Now an empty vector exists, reset this identifier.
    num_value_ = 0;
  }
  return *array_value_;
}

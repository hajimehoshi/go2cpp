...
Value Go::MakeFuncWrapper(int32_t id) {
  return Value{std::make_shared<Function>(
    [this, id](Value self, std::vector<Value> args) -> Value {
      // Cache the event to avoid being GCed unnecessarily at syscall/js.finalizeRef.
      // Note that the event information is never released. This is a known issue.
      ...
      if (it == cached_events_.end()) {
        ...
        cached_events_[id] = evt;
      } else {
        evt = it->second;
      }
      evt.ToObject().Set("this", self);
      evt.ToObject().Set("args", Value{args});
      pending_event_ = evt;
      Resume();
